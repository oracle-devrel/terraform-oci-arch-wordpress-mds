...
resource "oci_core_internet_gateway" "internet_gateway" {
  ...
  vcn_id         = oci_core_virtual_network.wpmdsvcn[0].id
  defined_tags   = { "${oci_identity_tag_namespace.ArchitectureCenterTagNamespace.name}.${oci_identity_tag.ArchitectureCenterTag.name}" = var.release }
}
...
resource "oci_core_subnet" "wp_subnet" {
  ...
  compartment_id             = var.compartment_ocid
  vcn_id                     = oci_core_virtual_network.wpmdsvcn[0].id
  route_table_id             = var.numberOfNodes > 1 ? oci_core_route_table.private_route_table[0].id : oci_core_route_table.public_route_table[0].id
  security_list_ids          = [oci_core_security_list.public_security_list_ssh[0].id, oci_core_security_list.public_security_list_http[0].id]
  dhcp_options_id            = oci_core_virtual_network.wpmdsvcn[0].default_dhcp_options_id
...
resource "oci_core_subnet" "lb_subnet_public" {
  count             = (!var.use_existing_vcn && var.numberOfNodes > 1) ? 1 : 0
  cidr_block        = cidrsubnet(var.vcn_cidr, 8, 0)
  display_name      = "lb_public_subnet"
  compartment_id    = var.compartment_ocid
  vcn_id            = oci_core_virtual_network.wpmdsvcn[0].id
  route_table_id    = oci_core_route_table.public_route_table[0].id
  security_list_ids = [oci_core_security_list.public_security_list_http[0].id]
  dhcp_options_id   = oci_core_virtual_network.wpmdsvcn[0].default_dhcp_options_id
  dns_label         = "lbpub"
  defined_tags      = { "${oci_identity_tag_namespace.ArchitectureCenterTagNamespace.name}.${oci_identity_tag.ArchitectureCenterTag.name}" = var.release }
}
...
resource "oci_core_subnet" "bastion_subnet_public" {
  count             = (!var.use_existing_vcn && var.numberOfNodes > 1 && var.use_bastion_service == false) ? 1 : 0
  cidr_block        = cidrsubnet(var.vcn_cidr, 8, 1)
  display_name      = "bastion_public_subnet"
  compartment_id    = var.compartment_ocid
  vcn_id            = oci_core_virtual_network.wpmdsvcn[0].id
  route_table_id    = oci_core_route_table.public_route_table[0].id
  security_list_ids = [oci_core_security_list.public_security_list_ssh[0].id]
  dhcp_options_id   = oci_core_virtual_network.wpmdsvcn[0].default_dhcp_options_id
...
resource "oci_core_subnet" "fss_subnet_private" {
  count                      = (!var.use_existing_vcn && var.numberOfNodes > 1 && var.use_shared_storage) ? 1 : 0
